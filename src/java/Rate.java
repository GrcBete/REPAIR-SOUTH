...
public class Rate extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int rated = Integer.parseInt(request.getParameter("rate"));
        
       HttpSession session=request.getSession();
        int Worker_Rated = Integer.parseInt((String)session.getAttribute("Worker_Rated")); 
        String database = "jdbc:mysql://localhost:3306/Services?allowPublicKeyRetrieval=true&useSSL=false";
        String user = "root";
        String password = "Silvabeth12";
        String mysql = "Select * from Rating where Worker_Id = "+Worker_Rated+"";
        
        try {
        Class.forName("com.mysql.cj.jdbc.Driver");
        
        Connection conn = DriverManager.getConnection(database, user, password);
        Connection conn1 = DriverManager.getConnection(database, user, password);
        PreparedStatement pst = conn.prepareStatement(mysql);
        ResultSet result = pst.executeQuery();
        if (result.next()){
        int rate = result.getInt(2);
        float newR = (rate+rated)/2;
        PreparedStatement pst1 = conn1.prepareStatement("update  Rating set Rating = ? where Worker_Id =?");
            pst1.setFloat(1,newR);
            pst1.setInt(2,Worker_Rated);
            pst1.executeUpdate();
            conn1.close();
